# simulates maximum tilted correlation
# using method 3.3
# requires package ppcor 

require(ppcor)

average_sim_max <- function(sims, n, p, Rmin, Rmax) {
  
  for (i in 1:sims) {
    
    cat("Beginning average simulation:", i)
    cat("\n")
    ...
    for (k in 1:length(R)) {
      xx <- c(xx, max(abs(average_tilt(W, R[k]) - diag(1,p))))
